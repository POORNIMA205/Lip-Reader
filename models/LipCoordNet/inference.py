...
def load_video(file, device: str):
    # Check if file is a directory (frames) or video file
    if os.path.isdir(file):
        ...
        # Video file - create the samples directory if it doesn't exist
        ...
        if result != 0:
            # Fallback: use OpenCV to extract frames
            ...
            while cap.isOpened():
                ...
                if not ret:
                    break
                frame_path = os.path.join(p, f"{frame_idx:04d}.jpg")
                cv2.imwrite(frame_path, frame)
                frame_idx += 1
            cap.release()
        
        files = os.listdir(p)
        files = sorted([f for f in files if f.endswith(('.jpg', '.jpeg', '.png'))], 
    ...
    points = []
    for I in tqdm(array, desc="Processing faces"):
        points.append(fa.get_landmarks(I))
...
def main():
    ...
    if os.path.isdir(args.input_video):
        samples_dir = args.input_video
    else:
        samples_dir = "samples"

    # generate lip coordinates
    coords = generate_lip_coordinates(samples_dir, detector, predictor)
